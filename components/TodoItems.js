import React from 'react'
import { IconContext } from 'react-icons'
import { MdDeleteForever } from 'react-icons/md'

const TodoItems = ({ elem, deleteItem, moveToDone }) => {
    return (
        <div className="flex items-center select-none my-2 dark:bg-terbg bg-tertxt rounded-md">
            <label
                htmlFor={`${elem._id}-${elem.status}`}
                className="group cursor-pointer p-3 flex-grow flex items-center rounded-md outline-none tracking-widest text-left"
                onClick={(e) => {
                    e.preventDefault()
                    moveToDone(elem._id, elem.status)
                }}
            >
                <input
                    id={`${elem._id}-${elem.status}`}
                    type={'checkbox'}
                    checked={elem.status}
                    className={'mr-3 form-checkbox rounded-full text-green-500'}
                    disabled
                />
                <p className="transition-opacity text-pribg group-hover:opacity-70 dark:text-pritxt">
                    {elem.status ? <strike>{elem.name}</strike> : elem.name}
                </p>
            </label>
            <span className="mx-1">|</span>
            <button
                className="p-1 m-2 rounded transition-opacity opacity-100 xl:hover:dark:bg-secbg xl:hover:bg-[#fff] xl:hover:scale-125 xl:hover:opacity-100"
                onClick={(e) => {
                    e.preventDefault()
                    deleteItem(elem._id)
                }}
            >
                <IconContext.Provider
                    value={{
                        color: 'red',
                        size: '1.2em',
                        className: 'global-class-name',
                    }}
                >
                    <MdDeleteForever />
                </IconContext.Provider>
            </button>
        </div>
    )
}

export default TodoItems
